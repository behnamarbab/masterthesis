...
The code coverage is a viable feature for detecting the new paths that were never executed during the fuzzing. Registering new paths for testing helps fuzzers in reaching to regions of the code that probable vulnerabilities exist. Later the fuzzer tests the input that caused the new path to check if fuzzing the input can reveal a new vulnerability after constructing tweaked inputs out of the current input. This loop of testing the different inputs continues until specific termination signals show up. Driller \cite{stephens2016driller} is a greybox fuzzer that tries digging into the applications's basic blocks, so that it reaches a specific region. Driller focuses on fuzzing more of the inputs that guide the execution as it gets closer to the specified region. Driller performs effective in testing a program for detecting errors in new patches of the application. Another graybox fuzzer, VUzzer \cite{rawat2017vuzzer} enhances the instrumentation to collect control- and data-flow features, which leads to guiding the agnostic fuzzer to find more \textit{intersting} inputs, with less effort (less trials of the fuzzed inputs). The key feature in a graybox fuzzer is the \textit{applicatoin agnosic} characteristic that targets any runable and observable program in the fuzzer's environment.
